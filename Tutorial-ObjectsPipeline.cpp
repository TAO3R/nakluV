...
void Tutorial::ObjectsPipeline::create(RTG& rtg, VkRenderPass render_pass, uint32_t subpass) {
	...
	{	// set4_LambertianCubemap: pre-convolved lambertian irradiance cubemap
		std::array< VkDescriptorSetLayoutBinding, 1> bindings{
			VkDescriptorSetLayoutBinding{
				.binding = 0,
				.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
	...
	{	// create pipeline
		...
		// an input assembly state structure that tells Vulkan that this pipeline will draw triangles
...
void Tutorial::ObjectsPipeline::destroy(RTG& rtg) {
	...
	{	// destory pipeline layout if not null
		vkDestroyPipelineLayout(rtg.device, layout, nullptr);
		layout = VK_NULL_HANDLE;
	}

	if (handle != VK_NULL_HANDLE)
	{	// destroy pipeline if not null
